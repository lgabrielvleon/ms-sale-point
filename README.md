...
```bash
ms-view-points$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ms-view-points$ sam local start-api
ms-view-points$ curl http://localhost:3000/
```
...
```bash
ms-view-points$ sam logs -n HelloWorldFunction --stack-name ms-view-points --tail
```
...
```bash
aws cloudformation delete-stack --stack-name ms-view-points
```
